#!/usr/bin/env python
"""
Load latest stock prices for Canadian tickers using SimFin Bulk API (shareprices).
...
- SIMFIN_MARKET (optional, default 'world')
...
def get_existing_latest_map(conn, use_timestamp: bool):
	"""Return a dict: symbol -> max(as_of) or max(latest_day) already stored for CA."""
	cur = conn.cursor()
	if use_timestamp:
		cur.execute(
			"""
...
def main():
	...
	if not tickers:
		print("[ingest_simfin_prices_ca] No Canadian tickers found in financials table.")
		# Log
		try:
			...
			cur2.execute(
				...
				(script_name,'warning','No Canadian tickers found in financials table', json.dumps({}), start_ts, now, int((now-start_ts)*1000))
	...
	if not api_key:
		print("[ingest_simfin_prices_ca] Missing SIMFIN_API_KEY")
		return 1
	market = get_env('SIMFIN_MARKET', 'world')
	variant = get_env('SIMFIN_PRICES_VARIANT', 'latest')  # 'latest' or 'daily'

	print(f"[ingest_simfin_prices_ca] Fetching SimFin shareprices dataset: market={market} variant={variant}")
	try:
	...
	# Optional datetime precision if provided by SimFin
	...
	ca_set = set(tickers)
	df = df[df['Ticker'].isin(ca_set)]
	if df.empty:
		print("[ingest_simfin_prices_ca] No matching CA tickers present in SimFin shareprices dataset.")
		return 0

	# Choose the latest row per ticker
	latest_rows = df.sort_values(['Ticker', 'Date', 'DateTime']).groupby('Ticker', as_index=False).tail(1)

	# Compute previous close if variant provides two days
	prev_map = {}
	...
	# Detect if table has as_of column for time precision
	...
	print(f"[ingest_simfin_prices_ca] Processing {total} Canadian tickers from SimFin")
	...
	for i, (_, row) in enumerate(latest_rows.iterrows()):
		try:
			...
			# Skip if we already have this as_of (or date) or newer stored
	...
	print(f"[ingest_simfin_prices_ca] Complete: {successful}/{total} prices loaded")
	# Log run status
